...
public class V5_Packet {
	...
	public V5_Packet(long routerIp, byte[] buf, int len) {
		...
		this.count = Utils.byte2int(buf, 2);
		...
		this.unix_nsecs = Utils.to_number(buf, 12, 4);
		this.flow_sequence = Utils.to_number(buf, 16, 4);
		...
		for (int i = 0, p = V5_Header_Size; i < this.count; i++, p += V5_Flow_Size) {
			...
			flow = new Netflow(routerIp, buf, p, this.unix_secs);
